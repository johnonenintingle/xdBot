...
LoadMacroLayer* LoadMacroLayer::create(geode::Popup* layer, geode::Popup* layer2, bool autosaves) {
	...
	if (ret->init(385, 291, Utils::getTexture().c_str()) && ret->setup(layer, layer2, autosaves)) {
		ret->autorelease();
		return ret;
	}
...
void LoadMacroLayer::onImportMacro(CCObject*) {
	// TODO: file picker needs async API update
	FLAlertLayer::create("Info", "File picker not yet ported", "OK")->show();
}
